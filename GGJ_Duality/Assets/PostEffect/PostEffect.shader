	...
	SubShader
	{
		...
		{ 
			...
			half4 frag ( v2f_img_custom i ) : SV_Target
			{
				...
				float4 color342 = IsGammaSpace() ? float4(0.2406121,0.3679245,0,0) : float4(0.04719842,0.1114872,0,0);
				...
				float4 color350 = IsGammaSpace() ? float4(0.02951098,0.09433961,0.01379494,0) : float4(0.002284131,0.009166724,0.00106772,0);
				...
				finalColor = ( ( ( (float4( 0,0,0,0 ) + (( ( lerpResult341 * break255.x ) + ( lerpResult343 * break255.y ) ) - float4( 0,0,0,0 )) * (temp_cast_3 - float4( 0,0,0,0 )) / (float4( 1,1,1,1 ) - float4( 0,0,0,0 ))) * temp_output_248_0 ) + ( ( ( 1.0 - step( grayscale357 , 0.0 ) ) - temp_output_248_0 ) * color350 ) ) + ( step( temp_output_248_0 , 0.0 ) * ( (0.0 + (temp_output_320_0 - 0.5) * (1.0 - 0.0) / (0.0 - 0.5)) * CalculateContrast(_Luminosity,float4( desaturateVar316 , 0.0 )) ) ) );
...
/*ASEBEGIN
Version=18912
2120;170;1779;970;-2285.893;2269.432;1;True;False
...
Node;AmplifyShaderEditor.ColorNode;342;1249.595,-2548.479;Inherit;False;Constant;_Color0;Color 0;12;0;Create;True;0;0;0;False;0;False;0.2406121,0.3679245,0,0;0,0,0,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.SimpleSubtractOpNode;362;2649.064,-1135.761;Inherit;True;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;349;3030.75,-1087.723;Inherit;False;2;2;0;FLOAT;0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
...
ASEEND*/
//CHKSM=9E2CC792B726A947D1C94CAC2D4281FF9A99FA49